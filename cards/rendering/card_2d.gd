...
func _draw():
	#TODO: there's a "renderHoverShadow" function in AbstractCard.java that we haven't done anything with
	
	#TODO: card bgs are not all the same size -- 
	
	#TODO: can skip all of this if card front is hidden
	...
	if(img_path):
		...
		#TODO: draw centered or not? cards are not uniform size, but some render ops appear to treat everything as uniform
		#AtlasHelper.CARDUI.draw_to_canvas_centered(bg_path,self,Vector2i(0,0))
		AtlasHelper.CARDUI.draw_to_canvas(bg_path,self,Vector2i(-308/2,-419/2))
		if(img_path!="SPECIAL"):		
			@warning_ignore("integer_division")
